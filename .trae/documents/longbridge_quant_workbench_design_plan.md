...
Longbridge 工作台首页“证券持仓明细”表按以下顺序展示：

| 列名 | 内容 | 展示规则 |
| --- | --- | --- |
| 标的 | `symbol`、`name` | 标的代码为主文本，名称为次级文本 |
| 数量 | `quantity` | 使用等宽数字；保留 Longbridge 返回的多空方向符号 |
| 市值 | `marketValue` | 使用持仓币种格式化，不应用盈亏色 |
| 成本 / 现价 | `averageCost`、`currentPrice` | 同一列上下对照展示，成本在上、现价在下，并分别显示“成本”“现价”标签 |
| 盈亏 | `unrealizedPnL` | 正数红色并显式带 `+`，负数绿色并保留 `-`，零值和不可用使用中性灰色 |
...
- 两个价格必须使用同一币种、相同小数精度和右对齐方式，便于纵向比较。
- 若接口返回的字符串已经带币种符号，则不得重复添加；币种不明确时使用持仓的 `currency` 字段。
- 任一值缺失时仅将对应行显示为“不可用”，不得以 `0` 代替，也不得隐藏整列。
- 成本和现价属于价格信息，统一使用中性色，不使用红绿盈亏色。
...
- 盈亏值统一保留两位小数，并使用等宽数字，避免刷新时列宽跳动。
- 正数必须补充前导 `+`；负数不得重复添加符号。
- 颜色遵循中国市场习惯：盈利为红色，亏损为绿色。
- 颜色语义只作用于盈亏数字，不作用于整行、标的、市值、成本或现价。
- 不可解析或不可用的盈亏显示“不可用”，使用中性灰色，禁止仅凭颜色暗示数值。
- 该规范复用 `.trae/documents/profit_font_navigation_design_draft.md` 的盈亏颜色 Token。
...
- 登录后：
  ...
  - 零值或不可用盈亏为中性灰色；成本、现价缺失时对应项显示“不可用”。
