  ...
  if (ctx.role === 'single') {
    ...
    if (o.action === 'HOLD') {
      ...
      if (['REDUCE_LONG', 'COVER_SHORT'].includes(effect)) {
        ...
        if (
          o.action === 'BUY'
          && f.policy.cashOpeningPolicy?.mode === 'AVAILABLE_CASH_ONLY'
          && (
            !Number.isFinite(f.account.availableCash)
            || o.limitPrice * o.orderQuantity > f.account.availableCash
          )
        ) {
          policyErrors.push('available_cash_exceeded')
        }
