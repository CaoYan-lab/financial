...
def main():
    ...
    if not account_id.isdigit():
        write_json(empty_response(False, account_id, page, page_size, start_date, end_date, ["SIMULATE accountId is required."]))
        return
    ...
    try:
        ...
        if ret != RET_OK or data is None:
            write_json(empty_response(False, account_id, page, page_size, start_date, end_date, [f"history_order_list_query failed: {data}"]))
            return
        ...
        write_json(empty_response(False, account_id, page, page_size, start_date, end_date, [f"Futu simulated order query unavailable: {exc}"]))
    finally:
